...
class Source(Base):
    ...
    source_type = Column(String(50), nullable=False)  # 'document', 'link', 'voice', 'message'
    ...
    __table_args__ = (
        Index('idx_collection_source', 'collection_id'),
    )
...
class Entity(Base):
    ...
    # Relationships
    relations = relationship("EntityRelation", back_populates="entity_a", foreign_keys="EntityRelation.entity_a_id")
    
    __table_args__ = (
        Index('idx_user_entity', 'user_id'),
    )
...
class EntityRelation(Base):
    ...
    entity_a_id = Column(UUID(as_uuid=True), ForeignKey("entities.entity_id"), nullable=False)
    entity_b_id = Column(UUID(as_uuid=True), ForeignKey("entities.entity_id"), nullable=False)
    ...
    # Relationships
    entity_a = relationship("Entity", back_populates="relations", foreign_keys=[entity_a_id])
    
    __table_args__ = (
        Index('idx_entity_relation', 'entity_a_id', 'entity_b_id'),
    )
...
class ChunkEntity(Base):
    ...
    chunk_id = Column(UUID(as_uuid=True), ForeignKey("memory_chunks.chunk_id"), nullable=False)
    ...
    __table_args__ = (
        Index('idx_chunk_entity', 'chunk_id', 'entity_id'),
    )
...
class Conversation(Base):
    ...
    __table_args__ = (
        Index('idx_user_conversation', 'user_id'),
    )
...
class CostTracking(Base):
    ...
    __table_args__ = (
        Index('idx_user_date_cost', 'user_id', 'date'),
    )
class AuditLog(Base):
    """Immutable audit log"""
...
class TelegramUpdateLog(Base):  # ← Only THIS one, at root level
    ...
    __table_args__ = (
        Index("idx_update_id_expires", "update_id", "expires_at"),
    )
