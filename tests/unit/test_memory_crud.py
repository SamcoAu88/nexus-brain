...
from src.models.memory import (
    UserProfile, Collection, Source, MemoryChunk, Conversation, Message
)
...
class TestCollectionCRUD:
    ...
    def test_list_collections_db(self, db_session, test_user, test_collection):
        ...
        collections = db_session.query(Collection).filter(
...
class TestChunkCRUD:
    ...
    def test_create_multiple_chunks(self, db_session, test_source):
        ...
        db_chunks = db_session.query(MemoryChunk).filter(
            MemoryChunk.source_id == test_source.source_id
    ...
    def test_list_chunks_ordered(self, db_session, test_source):
        ...
        chunks = db_session.query(MemoryChunk).filter(
            MemoryChunk.source_id == test_source.source_id
        ).order_by(MemoryChunk.chunk_index).all()
...
class TestCrossTableRelationships:
    ...
    def test_collection_source_relationship(self, db_session, test_collection, test_source):
    ...
    def test_source_chunk_relationship(self, db_session, test_source):
        ...
        source = db_session.query(Source).filter(
            Source.source_id == test_source.source_id
        ).first()
    ...
    def test_conversation_message_relationship(self, db_session, test_conversation):
        ...
        conversation = db_session.query(Conversation).filter(
            Conversation.conversation_id == test_conversation.conversation_id
        ).first()
