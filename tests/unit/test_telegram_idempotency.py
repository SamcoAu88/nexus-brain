...
class TestTelegramIdempotency:
    ...
    def test_first_update_stored(
        self, mock_ip, db_session, mock_telegram_request, valid_headers
    ):
        """Test: First webhook call stores update in database"""
        mock_ip.side_effect = mock_validate_telegram_ip

        response = client.post(
            "/api/telegram/webhook", json=mock_telegram_request, headers=valid_headers
        )
        ...
        stored = (
            db_session.query(TelegramUpdateLog)
            .filter(TelegramUpdateLog.update_id == 123456789)
            .first()
        )
    ...
    @patch("src.api.telegram_router.validate_telegram_ip")
    def test_duplicate_update_cached(
        self, mock_ip, db_session, mock_telegram_request, valid_headers
    ):
    ...
    @patch("src.api.telegram_router.validate_telegram_ip")
    def test_different_update_ids_processed(
        self, mock_ip, db_session, mock_telegram_request, valid_headers
    ):
        ...
        response1 = client.post(
            "/api/telegram/webhook", json=update1, headers=valid_headers
        )
        assert response1.status_code == 200
        ...
        response2 = client.post(
            "/api/telegram/webhook", json=update2, headers=valid_headers
        )
        assert response2.status_code == 200
    ...
    def test_missing_update_id_rejected(self, mock_ip, valid_headers):
        ...
        invalid_update = {"message": {"text": "Hello"}}  # Missing update_id

        response = client.post(
            "/api/telegram/webhook", json=invalid_update, headers=valid_headers
        )
    ...
    def test_invalid_secret_rejected(self, mock_ip, mock_telegram_request):
        ...
        response = client.post(
            "/api/telegram/webhook", json=mock_telegram_request, headers=invalid_headers
        )
    ...
    @patch("src.api.telegram_router.validate_telegram_ip")
    def test_expires_at_set_to_24h(
        self, mock_ip, db_session, mock_telegram_request, valid_headers
    ):
